...
class TaskController extends Controller {
    ...
    public function detail($id) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $message = trim($_POST['proposal_message'] ?? '');
            ...
            if ($applicationModel->exists($taskId, $_SESSION['user_id'])) {
                redirect('task/detail/' . $taskId . '?already_applied=1');
                exit();
            }
            ...
            $applicationModel->create([
                ...
                'proposal_amount' => $data['task']->budget, // Se usa el presupuesto ya definido
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                if ($action === 'approve') {
                    ...
                    if ($task) {
                        // Verificar si ya se hizo el pago previamente
                        $existingTransactions = $transactionModel->getUserTransactions($_SESSION['user_id']);
                        $alreadyPaid = false;

                        foreach ($existingTransactions as $txn) {
                            if ($txn->task_id == $taskId && $txn->type == 'pago_enviado') {
                                $alreadyPaid = true;
                                break;
                            }
                        }
                        ...
                        if (!$alreadyPaid) {
                            ...
                            // Marcar la tarea como completada y pagada
